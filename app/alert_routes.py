...
def _response(data: dict) -> StandardAgentResponse:
...
@router.get("", response_model=StandardAgentResponse)
async def list_alerts(limit: int = Query(100, ge=1, le=500), alert_type: Optional[str] = None):
    return _response({"events": alert_service.list_events(limit=limit, alert_type=alert_type)})


@router.get("/summary", response_model=StandardAgentResponse)
async def alert_summary():
    return _response(alert_service.summary())


@router.post("/queue-status", response_model=StandardAgentResponse)
async def queue_status_alert(payload: QueueStatusAlertRequest):
    ...
    return _response({"alert_emitted": event is not None, "event": event})


@router.post("/reconciliation", response_model=StandardAgentResponse)
