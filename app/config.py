...
BACKTEST_AGENT_URL = os.getenv("BACKTEST_AGENT_URL", "http://backtest-agent:8016")
BACKTEST_AGENT_TIMEOUT = int(os.getenv("BACKTEST_AGENT_TIMEOUT", 20))
BACKTEST_AGENT_ENABLED = _env_bool("BACKTEST_AGENT_ENABLED", True)
BACKTEST_EXECUTION_GATE_REQUIRED = _env_bool(
    "BACKTEST_EXECUTION_GATE_REQUIRED", False
)
BACKTEST_GATE_SKILL_ID = os.getenv(
    "BACKTEST_GATE_SKILL_ID", "hourly-sma-crossover"
).strip()
BACKTEST_GATE_STRATEGY_ID = os.getenv(
    "BACKTEST_GATE_STRATEGY_ID", "hourly-sma-crossover"
).strip()
...
BACKTEST_WALK_FORWARD_GATE_REQUIRED = _env_bool(
...
INVESTABILITY_MIN_MARKET_CAP_USD = float(
    os.getenv("INVESTABILITY_MIN_MARKET_CAP_USD", "300000000")
)
INVESTABILITY_MIN_AVG_DOLLAR_VOLUME_USD = float(
    os.getenv("INVESTABILITY_MIN_AVG_DOLLAR_VOLUME_USD", "5000000")
)
INVESTABILITY_MAX_SPREAD_BPS = float(
    os.getenv("INVESTABILITY_MAX_SPREAD_BPS", "100")
)
INVESTABILITY_MAX_ATR_PCT = float(
    os.getenv("INVESTABILITY_MAX_ATR_PCT", "15")
)
INVESTABILITY_REQUIRE_AVG_DOLLAR_VOLUME = _env_bool(
    "INVESTABILITY_REQUIRE_AVG_DOLLAR_VOLUME", False
)
INVESTABILITY_REQUIRE_SPREAD = _env_bool("INVESTABILITY_REQUIRE_SPREAD", False)
INVESTABILITY_REQUIRE_ATR = _env_bool("INVESTABILITY_REQUIRE_ATR", True)
INVESTABILITY_BLOCK_EXTREME_VOLATILITY = _env_bool(
    "INVESTABILITY_BLOCK_EXTREME_VOLATILITY", True
)
...
if TRADING_MODE == "LIVE" and PROFIT_DECISION_EXECUTION_ENABLED:
    raise RuntimeError(
        "PROFIT_DECISION_EXECUTION_ENABLED is limited to PAPER/SIMULATOR during rollout."
    )
