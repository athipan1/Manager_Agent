...
def _process_agent_response(resp: Union[StandardAgentResponse, Dict[str, Any]], agent_type: str) -> Optional[ReportDetail]:
    ...
    if not data_obj:
        report_logger.warning(f"Invalid data payload for {agent_type}: {resp_dict}")
        return None
...
async def _execute_trade(exec_client: ExecutionAgentClient, trade_decision: dict, account_id: Union[int, str], correlation_id: str) -> dict:
    ...
    try:
        ...
        if response.status.upper() in ["PENDING", "PLACED", "EXECUTED"]:
            report_logger.info(
                f"Successfully submitted order for {ticker}. Order ID: {response.order_id}, "
...
async def _analyze_single_asset(ticker: str, correlation_id: str) -> dict:
    ...
    status_value = "complete" if tech_detail and fund_detail else "partial"
...
def _fundamental_v2_data(analysis_result: Dict[str, Any]) -> Dict[str, Any]:
    return (analysis_result.get("raw_data", {}).get("fundamental") or {}).get("data") or {}


def _fundamental_v2_scores(analysis_result: Dict[str, Any]) -> Dict[str, Any]:
    data = _fundamental_v2_data(analysis_result)
    factor_keys = [
        "quality_score",
        "growth_score",
        "valuation_score",
    ...
    factor_scores = {key: _normalize_score(data.get(key)) for key in factor_keys}
    non_zero = [value for value in factor_scores.values() if value > 0]
    composite = _normalize_score(data.get("confidence_score"))
    if composite == 0 and non_zero:
        composite = sum(non_zero) / len(non_zero)
    return {
        **factor_scores,
...
def _score_deep_analysis(analysis_result: Dict[str, Any], scanner_score: float) -> Dict[str, Any]:
    ...
    raw_fund_score = _normalize_score(fund_detail.score if fund_detail else 0.0)
    fundamental_v2 = _fundamental_v2_scores(analysis_result)

    v2_composite = fundamental_v2.get("composite_score", 0.0)
    if v2_composite > 0:
        fund_score = v2_composite
        fund_source = "fundamental_agent_v2"
    elif raw_fund_score > 0:
        fund_score = raw_fund_score
        fund_source = "fundamental_agent"
    else:
        fund_score = scanner_score
        fund_source = "scanner_fallback"

    verdict = analysis_result.get("final_verdict", "hold")
    verdict_score_map = {"strong_buy": 1.00, "buy": 0.80, "hold": 0.45, "sell": 0.10, "strong_sell": 0.00}
    ...
    return {
        ...
        "fundamental_score_source": fund_source,
        "quality_score": round(fundamental_v2.get("quality_score", 0.0), 4),
        "growth_score": round(fundamental_v2.get("growth_score", 0.0), 4),
        "valuation_score": round(fundamental_v2.get("valuation_score", 0.0), 4),
        "financial_health_score": round(fundamental_v2.get("financial_health_score", 0.0), 4),
        "cash_flow_score": round(fundamental_v2.get("cash_flow_score", 0.0), 4),
        "sector": fundamental_v2.get("sector"),
        "risk_flags": fundamental_v2.get("risk_flags", []),
        "comparative_analysis": fundamental_v2.get("comparative_analysis", {}),
    ...
    try:
        ...
        if not candidates:
            return StandardAgentResponse(
                ...
                version="1.0.0",
                timestamp=datetime.datetime.now(datetime.UTC),
                data={"report_id": correlation_id, "stage": "scanner_discovery", "message": "Scanner returned zero candidates.", "scanner_error": scan_response.error, "scanner_data": scan_payload},
        ...
        if not valid_results:
            return StandardAgentResponse(
                ...
                version="1.0.0",
                timestamp=datetime.datetime.now(datetime.UTC),
                data={"report_id": correlation_id, "stage": "deep_analysis", "scanner_candidates": selected_tickers, "analysis_results": analysis_results},
        ...
        return StandardAgentResponse(
            ...
            data={
                ...
                "winner": {
                    ...
                    "fundamental_v2": _fundamental_v2_scores(winner_analysis),
                },
                "ranked_candidates": [
                    {
                        "rank": index + 1,
        ...
        async with ScannerAgentClient() as scanner_client:
            ...
            if not selected_tickers:
                ...
                multi_report = MultiOrchestratorResponse(
                    multi_report_id=correlation_id,
                    timestamp=datetime.datetime.now(datetime.UTC),
                    execution_summary=ExecutionSummary(total_trades_approved=0, total_trades_executed=0, total_trades_failed=0),
                    results=[],
                )
                return StandardAgentResponse(status="success", agent_type="manager-agent", version="1.0.0", timestamp=datetime.datetime.now(datetime.UTC), data=multi_report)
