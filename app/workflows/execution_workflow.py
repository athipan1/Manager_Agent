...
def _normalized_status(value: Any) -> str:
    """Normalize string or Enum-like statuses to plain uppercase tokens."""
...
async def execute_trade(
    ...
    try:
        ...
        response_status = _normalized_status(response.status)
        if response_status in ["PENDING", "PLACED", "EXECUTED"]:
            return {
                "status": "submitted",
        ...
        return {
            "status": "rejected",
            "risk_approval_id": risk_approval_id,
            "reason": f"Execution Agent returned status: {response_status}",
        }
