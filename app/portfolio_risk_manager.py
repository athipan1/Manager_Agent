...
def _position_price(position: Any | None) -> Decimal:
    if not position:
        return Decimal("0")
    return _as_decimal(_get(position, "current_market_price", "current_price", "average_cost", "avg_entry_price", default=0))


def _position_exposure(position: Any | None) -> Decimal:
    if not position:
        return Decimal("0")
...
class PortfolioRiskManager:
    def __init__(self, portfolio_value: Decimal, existing_positions: List[Position], per_request_risk_budget: Decimal, max_total_exposure: Decimal, min_position_value: Decimal):
        ...
        self.current_exposure = sum(_position_exposure(p) for p in self.existing_positions)
    ...
    def evaluate_sell(self, sell_decision: Dict[str, Any]):
        if not (sell_decision.get("approved") and sell_decision.get("action") in ["sell", "strong_sell"]):
            return
        position_to_sell = next((p for p in self.existing_positions if _position_symbol(p) == str(sell_decision["symbol"]).upper()), None)
        if position_to_sell:
            sell_value = Decimal(sell_decision["position_size"]) * _position_price(position_to_sell)
