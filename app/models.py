...
class CreateOrderRequest(BaseModel):
    """
    Payload to send to the Execution Agent to create a new order.
    """
    symbol: str = Field(..., description="The stock symbol, e.g., 'AAPL'")
    side: Literal["buy", "sell", "short", "cover"] = Field(..., description="The side of the order")
    quantity: Decimal = Field(..., gt=0, description="The number of shares to trade")
    price: Decimal = Field(..., gt=0, description="The target price for the order")
    client_order_id: UUID = Field(..., description="The unique ID generated by the Manager Agent for idempotency")


class CreateOrderResponse(BaseModel):
    """
    Expected response from the Execution Agent after creating an order.
    """
    status: str = Field(..., description="The status of the order, e.g., 'PENDING', 'PLACED'")
    order_id: str = Field(..., description="The unique order ID from the execution system")
    client_order_id: UUID = Field(..., description="The original client_order_id sent in the request")
