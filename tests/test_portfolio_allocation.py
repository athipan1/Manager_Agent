...
def test_classify_value_rebound_from_low_valuation():
    value = _candidate(
        "VALUE",
        raw_scores={"pe_ratio": 12, "pb_ratio": 1.2},
    )
...
def test_conflicting_high_confidence_evidence_is_quarantined():
    ...
    assert set(decision.conflict_buckets) >= {
        CORE_DIVIDEND,
        NEWS_MOMENTUM,
    }
...
def test_unknown_scanner_bucket_is_rejected_without_heuristic_fallback():
    invalid = _candidate(
        ...
        tags=["dividend"],
        metadata={
            "primary_strategy_bucket_hint": "ticker_specific_bucket"
        },
    )
...
def test_allocation_plan_assigns_candidates_to_buckets():
    ...
    assert [
        candidate["symbol"]
        for candidate in plan["buckets"][CORE_DIVIDEND]["candidates"]
    ] == ["KO"]
    assert [
        candidate["symbol"]
        for candidate in plan["buckets"][VALUE_REBOUND]["candidates"]
    ] == ["VALUE"]
    assert [
        candidate["symbol"]
        for candidate in plan["buckets"][NEWS_MOMENTUM]["candidates"]
    ] == ["NEWS"]
    assert (
        plan["buckets"][CORE_DIVIDEND]["candidates"][0][
            "suggested_max_value"
        ]
        == 10000.0
    )
    assert (
        plan["buckets"][VALUE_REBOUND]["candidates"][0][
            "suggested_max_value"
        ]
        == 7000.0
    )
    assert (
        plan["buckets"][NEWS_MOMENTUM]["candidates"][0][
            "suggested_max_value"
        ]
        == 3000.0
    )
...
def test_allocation_plan_quarantines_unclassified_candidate():
    ...
    assert (
        plan["quarantine"][0]["blocked_reason"]
        == "strategy_bucket_not_auto_approved"
    )
