...
def test_projects_decision_path_and_safe_candidate_reasons(tmp_path):
    write_json(tmp_path / "hourly-preflight.json", {"portfolio_cycle_id": "cycle-1", "correlation_id": "corr-1"})
    write_json(tmp_path / "hourly-position-review.json", {
        "generated_at": "2026-08-09T11:45:49Z",
        "market_regime": {"regime": "bull", "risk_level": "low", "confidence_score": 0.77},
        "position_decisions": [],
        "safe_for_candidate_analysis": True,
    })
    write_json(tmp_path / "hourly-pre-backtest-discovery.json", {
        "status": "success",
        ...
        "response": {"data": {
            ...
            "ranked_candidates": [{
                "symbol": "BANX", "rank": 1, "final_verdict": "buy",
                "strategy_bucket": "value_rebound", "allows_new_entry": True,
    ...
    observability = enrich_snapshot(snapshot(), tmp_path)["observability"]
    ...
    assert current["stages"][2]["summary"] == {"regime": "bull", "riskLevel": "low", "confidence": 0.77}
    ...
    assert '"market_cap": 123' not in serialized
    assert '"current_price"' not in serialized
    assert observability["lastMeaningful"] == current


def test_maps_backtest_and_risk_rejection_without_raw_diagnostics(tmp_path):
    ...
    write_json(tmp_path / "hourly-pre-backtest-discovery.json", {
        ...
        "response": {"data": {
            "scanner_count": 1,
            "ranked_candidates": [{
                "symbol": "AAPL", "rank": 1, "final_verdict": "buy",
                "allows_new_entry": True, "evidence_gate_passed": True,
                "strategy_bucket": "core_dividend",
                "score_breakdown": {"final_opportunity_score": 0.8},
            }],
            "risk_approvals": [{
                "symbol": "AAPL", "approved": False, "status": "risk_rejected",
    ...
    write_json(tmp_path / "hourly-backtest-result.json", {
        "data": {"items": [{
            "symbol": "AAPL", "status": "eligible_strategy_found",
            "result": {"raw_trades": [1, 2, 3]},
        }]},
    })

    current = enrich_snapshot(snapshot("risk_rejected"), tmp_path)["observability"]["current"]
    candidate = current["candidates"][0]
    assert candidate["status"] == "blocked"
    assert candidate["stageReached"] == "risk"
    assert candidate["reasonCodes"] == ["max_position_exposure"]
    assert current["stages"][1]["summary"] == {"attemptedCount": 1, "eligibleCount": 1}
...
def test_metadata_cycle_preserves_last_meaningful(tmp_path):
    previous_cycle = {
        "source": "hourly_artifact", "flowKind": "decision_path",
        "correlationId": "corr-old", "cycleId": "cycle-old", "workflowRunId": 99,
        "observedAt": "2026-08-09T10:00:00Z", "status": "controlled_no_trade",
        "reasonCode": "market_closed", "stages": [], "candidates": [{"symbol": "AAPL"}],
    }
    ...
    current_snapshot = snapshot("scheduled_paper_cycle_not_authorized")
    current_snapshot["cycle"]["id"] = None
    current_snapshot["workflow"]["conclusion"] = "skipped"

    observability = enrich_snapshot(current_snapshot, tmp_path, previous)["observability"]
...
def test_sensitive_reason_code_is_redacted(tmp_path):
    ...
    write_json(tmp_path / "hourly-pre-backtest-discovery.json", {
        ...
        "response": {"data": {
            "scanner_count": 1,
            "ranked_candidates": [{
                "symbol": "TEST", "rank": 1, "final_verdict": "buy",
                "allows_new_entry": True, "evidence_gate_passed": True,
                "score_breakdown": {"final_opportunity_score": 0.7},
                "investability_gate": {"allowed": False, "rejection_codes": ["api_key=super-secret"]},
            }],
        }},
    })
    current = enrich_snapshot(snapshot(), tmp_path)["observability"]["current"]
    assert current["candidates"][0]["reasonCodes"] == ["redacted"]
    assert "super-secret" not in json.dumps(current).lower()
