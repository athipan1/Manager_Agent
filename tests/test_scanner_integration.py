...
@patch('app.main.LearningAgentClient', autospec=True)
@patch('app.main.config_manager')
def test_scan_and_analyze_with_pydantic_model(mock_cm, mock_learning, mock_db, mock_execute, mock_analyze, mock_scanner):
    # Setup config
    mock_cm.get.side_effect = lambda key, default=None: {
        'PER_REQUEST_RISK_BUDGET': '0.25', 'RISK_PER_TRADE': '0.01',
        'STOP_LOSS_PERCENTAGE': '0.10', 'MAX_POSITION_PERCENTAGE': '0.2',
    ...
    # Mock Scanner returning a Pydantic model
    mock_scanner_instance = mock_scanner.return_value.__aenter__.return_value
    scanner_data = ScannerResponseData(
        candidates=[
            ScannerCandidate(symbol="TSLA", recommendation="STRONG_BUY", confidence_score=0.9),
            ScannerCandidate(symbol="GOOG", recommendation="BUY", confidence_score=0.7)
        ],
        scan_type="technical",
        count=2
    )
    ...
    mock_analyze.side_effect = [
        mock_analysis_result("TSLA", "buy"),
        mock_analysis_result("GOOG", "buy")
    ]
