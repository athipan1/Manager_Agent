...
from scripts.run_profitability_funnel_audit import (
    build_upstream_failure_funnel,
    run_audit,
)
...
def test_runner_writes_audit_artifact_when_discovery_is_missing(tmp_path: Path):
    ...
    assert funnel["primary_bottleneck"]["stage"] == "upstream_runtime"
    assert "Scanner preselection was not reached" in markdown.read_text(
        encoding="utf-8"
    )
...
def test_runner_uses_normal_funnel_when_discovery_exists(tmp_path: Path):
    ...
    discovery.write_text(
        '{"status":"success","request":{"max_universe":100},'
        '"backtest_symbols":["AAPL"],"response":{"data":{'
        '"scanner_metadata":{"attempted_count":100,"analyzed_count":95},'
        '"scanner_count":1,"deep_analysis_count":1,'
        '"ranked_candidates":[{"symbol":"AAPL","strategy_bucket":"core_dividend",'
        '"evidence_gate_passed":true}],'
        '"bucket_selection":{"summary":{"selected_before_investability":1,'
        '"selected_after_investability":1}},'
        '"allocation_plan":{"investability_gate":{"rejected":[]}},'
        '"exposure_gate":{"summary":{"allowed_count":1}}}}}',
        encoding="utf-8",
    )
