...
def test_upstream_failure_is_reported_without_fake_scanner_metrics(tmp_path: Path):
    funnel = build_upstream_failure_funnel(
        upstream_report={"cycleStatus": "failure"},
        source_run_id="31942008720",
    ...
    assert funnel["safety"]["production_opportunity_threshold_relaxed"] is False
...
def test_runner_writes_audit_artifact_when_discovery_is_missing(tmp_path: Path):
    ...
    assert "Scanner preselection did not complete successfully" in rendered
    assert "never authorizes Shadow broker orders" in rendered
...
def test_runner_uses_normal_funnel_when_discovery_exists(tmp_path: Path):
    ...
    discovery.write_text(
        '{"status":"success","request":{"max_universe":100},"backtest_symbols":["AAPL"],"response":{"data":{"scanner_metadata":{"attempted_count":100,"analyzed_count":95},"scanner_count":1,"deep_analysis_count":1,"ranked_candidates":[{"symbol":"AAPL","strategy_bucket":"core_dividend","evidence_gate_passed":true}],"bucket_selection":{"summary":{"selected_before_investability":1,"selected_after_investability":1}},"allocation_plan":{"investability_gate":{"rejected":[]}},"exposure_gate":{"summary":{"allowed_count":1}}}}}',
        encoding="utf-8",
    )
