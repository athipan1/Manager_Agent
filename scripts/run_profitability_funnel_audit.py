...
def build_upstream_failure_funnel(*, upstream_report: dict[str, Any] | None, source_run_id: str | None, source_run_conclusion: str | None) -> dict[str, Any]:
    upstream = upstream_report or {}
    conclusion = str(source_run_conclusion or "unknown").lower()
    cycle_status = str(upstream.get("cycleStatus") or upstream.get("cycle_status") or upstream.get("status") or "unavailable")
    ...
    bottleneck = {
        ...
        "recommended_action": "Repair the earliest failing Hourly runtime phase before tuning Scanner, Backtest, Investability, Risk, or Execution thresholds.",
    }
    return {
        ...
        "stages": [{"name": "hourly_runtime", "count": 1}, {"name": "scanner_preselection_reached", "count": 0}],
        "health": {
        ...
        "upstream": {"hourly_cycle_status": cycle_status, "discovery_artifact_present": False},
        "safety": {"risk_thresholds_relaxed": False, "investability_thresholds_relaxed": False, "purpose": "diagnostic_only"},
    }
...
def run_audit(*, discovery_path: Path, upstream_report_path: Path | None, output_path: Path, markdown_path: Path | None, source_run_id: str | None, source_run_conclusion: str | None) -> dict[str, Any]:
    if discovery_path.exists():
        ...
        funnel = build_upstream_failure_funnel(upstream_report=_read_json(upstream_report_path), source_run_id=source_run_id, source_run_conclusion=source_run_conclusion)
        markdown = render_upstream_markdown(funnel)

    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(json.dumps(funnel, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build a profitability funnel or an upstream-runtime diagnostic.")
    ...
    funnel = run_audit(discovery_path=args.discovery, upstream_report_path=args.upstream_report, output_path=args.output, markdown_path=args.markdown, source_run_id=args.source_run_id, source_run_conclusion=args.source_run_conclusion)
    print(json.dumps({"source_status": funnel.get("source_status"), "primary_bottleneck": _dict(funnel.get("primary_bottleneck")).get("stage")}, sort_keys=True))
    return 0
